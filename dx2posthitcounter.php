...
{
	wp_register_style( 'dx2hits_maincss', plugins_url('/style.css?v=1.0.5', __FILE__), false, '1.0.5' );
...
##										ACTIVATION HOOKS
################################################################################################
function dx2posthitcounter_dbsetup($network_wide) {
	global $wpdb;
...
function dx2hits_dashboard_scripts($hook)
{
	...
	if($screen->base == "dashboard" && $hook == "index.php")
	{
		...
		wp_register_script( 'dx2hits_dashwidget', plugins_url('/scripts/dashboardwidget.js?v='.time(), __FILE__), false, '1.1.0' );
...
#region multisite widget
function dx2hits_multihitreport() {
	wp_add_dashboard_widget(
                 'dx2_sitehits',         // Widget slug.
                 'Network Visitor Summary',         // Title.
                 'dx2hits_multisitehithistory' // Display function.
        );	
}
add_action( 'wp_network_dashboard_setup', 'dx2hits_multihitreport' );
function dx2hits_multisitehithistory()
{
	?><canvas id="networkhitcanvas"></canvas><?php	
	$labels = array();
	$datasetsdata = array();
	$sites = get_sites();	
	$todaytotals = array();
	...
	foreach($sites as $site)
	{
		...
		{		
			$date = date("Y-m-d", strtotime('-'. $i .' days'));
			$daycount = $wpdb->get_results ("SELECT hitcount FROM ".$tablename." WHERE itemkey = 'hitsday_".$date."'", 'ARRAY_N');
			$hits = 0;
			foreach($daycount as $postcount) $hits = $hits + $postcount[0];
			$hitsbyday[] = array($date, $hits);
		}
		$hitsbyday = array_reverse($hitsbyday);
		
		$todaytotals[$site->blog_id] = $hitsbyday[$graphlimit-1][1];
		
		if(count($labels) == 0)
		{
			for($i = 0; $i < $graphlimit; $i++)
			{		
				$labels[$i] = date("M j", strtotime($hitsbyday[$i][0]));
			}
		}
		
		for($i = 0; $i < $graphlimit; $i++)
		{		
			$datasetsdata[$i] = $datasetsdata[$i] + $hitsbyday[$i][1];
		}
	}
	restore_current_blog();
	$dashvars = array(
	...
	wp_register_script( 'dx2hits_dashwidget', plugins_url('/scripts/dashboardwidget.js?v='.time(), __FILE__), false, '1.1.0' );
	wp_localize_script( 'dx2hits_dashwidget', 'networkdashwidget', $dashvars );
	wp_enqueue_script( 'dx2hits_dashwidget' );
	
	?><strong>Totals For Today</strong><br><hr>
	<table id="networkblogreport"><tr><th>Blog Path</th><th>Views</th></tr>
	<?php
	foreach($sites as $site)
	{
		?>
		<tr><td><a href="<?php echo $site->path;?>wp-admin"><?php echo $site->path; ?></a></td><td><?php echo $todaytotals[$site->blog_id];?></td></tr>
		<?php
	}
	echo "</table>";
}
...
class dx2hits_popular_posts_widget extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		echo "<strong>".$instance['title']."</strong>";
		...
		if($posts != null && count($posts) != 0) {
			foreach($posts as $post)
			{
				...
				$outputhtml .= "<a href='".get_site_url()."/".$post->post_name."'>".$post->post_title;
